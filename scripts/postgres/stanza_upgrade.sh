...
if [[ -z "$pg_version" ]]; then
  echo "ERROR: Database not reachable. Abort Stanza-upgrade attempt" >&2
  exit 1
fi
...
if [[ -z "$stanza_version" ]]; then
  echo "ERROR: Not able to recieve stanza-version. Abort Stanza-upgrade attempt" >&2
  exit 1
fi
...
if [ "$pg_version" != "$stanza_version" ]; then
  ...
  if pgbackrest stanza-upgrade --stanza=db; then
    ...
    echo "ERROR: stanza upgrade failed!" >&2
    exit 1
  fi
else
